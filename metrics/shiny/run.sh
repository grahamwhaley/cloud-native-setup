...
IMAGE="${IMAGE:-metrics-shiny}"
DOCKERFILE="${SCRIPT_PATH}/shiny_dockerfile/Dockerfile"
PORT=3838
...
HOSTAPPDIR="${SCRIPT_PATH}/apps"
GUESTAPPDIR="/srv/shiny-server/metrics"
...
build_dockerfile_image()
{
	...
	if ! docker build --label "$image" --tag "${image}" -f "$dockerfile_path" "$dockerfile_dir"; then
		echo "Failed to docker build image $image"
		exit 1
	fi
}
...
setup() {
	...
	if [ $check_subdir -eq 0 ]; then
		echo "No subdirs in [${HOSTINPUTDIR}] to read results from."
		exit 1
	fi
...
run() {
	echo "Attach to: http://localhost:${PORT}/metrics/"
	docker run ${extra_opts} --rm -p ${PORT}:${PORT} -v ${HOSTSCRIPTDIR}:${GUESTSCRIPTDIR} -v ${HOSTINPUTDIR}:${GUESTINPUTDIR} -v ${HOSTAPPDIR}:${GUESTAPPDIR} ${extra_volumes} ${IMAGE} ${extra_command}
	ls -la ${HOSTOUTPUTDIR}/*
}
